...
app.post('/api/generate', async (req, res) => {
    ...
    try {
        const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
        const result = await model.generateContent({ prompt: formattedPrompt });
        const response = result.responses[0].text;

        // Log the raw response to inspect what's being returned
        console.log("Raw response from Generative AI:", response);
